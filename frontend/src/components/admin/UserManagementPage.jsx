...
import {
  ...
  TextField, FormControl, InputLabel, Select, MenuItem, Grid, Stack, Tooltip
} from '@mui/material';
  ...
  const fetchUsers = async (currentPage, limit, role, faculty, banned, keyword, isInitialLoad) => {
    ...
    try {
        ...
        setUsers(data.users.map(u => ({
          ...
          cost_month_year: u.cost_month_year,
      })));
        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              {isAdmin && <TableCell><b>Chi phí tháng</b></TableCell>}
          ...
          <TableBody>
            {users.map((user) => (
              <TableRow key={user.id} hover>
                ...
                {isAdmin && (
                    <TableCell>
                        <Tooltip 
                            title={user.cost_month_year ? `Tháng: ${user.cost_month_year}` : 'Chưa có dữ liệu'} 
                            arrow
                        >
                            <Chip
                                icon={<AttachMoneyIcon sx={{ fontSize: 16 }} />}
                                label={`$${(user.monthly_cost || 0).toFixed(4)}`}
                                size="small"
                                color="info"
                                variant="outlined"
            ...
            {users.length === 0 && !isInitialLoading && ( 
              <TableRow>
                <TableCell colSpan={isAdmin ? 7 : 6} align="center" sx={{ color: 'text.secondary', py: 4 }}>
